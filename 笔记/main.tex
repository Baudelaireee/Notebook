...
\begin{enumerate}[(a)]
    ...
    \item The Zariski topology is a \(T_1\)-space, i.e. any single point is closed. Indeed, and point \(a \in k^n\) can be seen uniquely as a maximal ideal \(\mathfrak{m}_a = (X_1 - a_1,...,X_n - a_n)\) by nullstellensatz.
    
    \item The Zariski topology is quasi-compact, i.e. any open cover has a finite subcover. Indeed, let \(\{U_i\}_{i \in I}\) be an open cover of \(k^n\), then by taking complements we have
\end{enumerate}
\newpage
...
To complete the functoriality, we consider how can we define 
