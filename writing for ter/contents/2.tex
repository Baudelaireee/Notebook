...
\begin{definition}
    A Hecke character of \(K\) is a continous homomorphism \(\chi: \adele_K^\times \to \cc^\times\) such that \(\chi|_{K^\times} = 1\). 
\end{definition}
...
By product formula in proposition \ref{adele-norm}, we can see that \(|\cdot|^s_{\adele}\) defines a Hecke character for any \(s \in \cc\), similarly a decomposition of Hecke character exists as following.
...
\end{proof}
    ...
    Similarly, two Hecke chracters \(\chi\) and \(\chi'\) on class group \(A^\times /K\) are called equivalent if they concide on the compact subgroup \(A^\times_1/K\), then each equivalence class consists of Hecke characters of the form \(\eta|\cdot|_\adele^s\) with \(\eta\) a fixed unitary character, and a parametrization can be done to identify each class with complex plane, which shows that the set of Hecke characters has the structure of Riemann surface with countable connected components homeomorphic to \(\cc\).
