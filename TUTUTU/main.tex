...
\[ \forall A \in \mathcal{V}, \quad e \in A \land A \subset G\]
...
\item In particular, if the orbit of the action is unique, then we say that the orbit is \textbf{transitive}, i.e. for any \(x,y \in X\), there is some \(g \in G\) such that \(g * x = y\). Formally, we call a space \(X\) as a \textbf{G-homogeneous} space if \(G\) acts on \(X\) transitively.
...
\begin{example}
    ...
    Then the orbit of \((x,y)\) is \(\{(x,y),(-x,-y)\}\), and the action is free too. Similarly with (2), we can see the quotient space \(\sph^1/\sim\) as \(\rp^1\), the more details of topology will be discussed later.
...
In this section, we suppose that readers have some basic knowledge of differential manifold, for example the chart, altas and so on, it can be founed in chapter 1,2 of Lee's book <<Introduction to Smooth Manifolds>>. The Lie group is a natural combination of topological group and differential manifold:
\begin{definitionplain*}
    A Lie group is a group \((G,*,\mathcal{T})\) satisfies:\\
    (1) \((G,*)\) is a group.\\
    (2) \((G,\mathcal{T})\) is a smooth manifold.\\
    (3) The group operation is compatiable with the smooth structure, i.e. the map
    \[G\times G \to G, \quad (g,h) \mapsto g*h^{-1}\]
    is a smooth map, where \(G \times G\) is equipped with the product smooth structure.
\end{definitionplain*}
\begin{remark}
    Here are some immediate results from topological group and differential manifold, the verifcation is not difficult, I tend to give them as a definition here:\\

    (1) The \textbf{morphism} of Lie group is a group homomorphsim which is also a smooth map; The \textbf{isomorphism} of Lie group is a group isomorphism which is also a diffeomrphsim. Which motivates us to define the category
    \[\cat{LieGrp}{\text{Lie group}}{\text{morphism of Lie group}}\]

    (2) The subgroup of a Lie Group is \textbf{not} always a Lie group: consider additive group \(\qq \subset \rr\) with the usual topology, here we know that \(\qq\) is not a manifold. A sufficient condition is that \textbf{closed} subgroup of Lie group is a Lie group \textbf{(Cartan's Theorem)}.

    (3) The translation of the group (by left or right) is a diffeomrphsim:
    \[L_g :G \to G, \quad x \mapsto g*x\] 
    and
    \[R_g: G \to G, \quad x \mapsto x*g\]

    (4) The product of two Lie group is still a Lie group under the product smooth structure and the component-wise group operation.

    (5) The Lie group is always \textbf{Hausdorff} and \textbf{second countable}, since the smooth manifold structure forces it.
\end{remark}

There are many examples of Lie group, it is a very rich area to study, here we give a proof that \textbf{The circle \(\sph^1\) is a Lie group}, it is a important example in topological group.

\begin{proof}
    The complete proof under \(\rr^2\) is a bit long and boring, it is better to consider a natural real manifold stucture \(\cc\), the unique chart is given by homemomorphism
    \[\phi:\cc \to \rr^2, \quad z = x + iy \mapsto (x,y)\]
    The uniqe transition map is \(\phi \circ \phi^{-1} = id_{\rr^2}\), so it is indeed a smooth manifold.

    Then it is not diffcult to verify a smooth map
    \[\mu: \cc^* \times \cc^* \to \cc^*, \quad (z,w) \mapsto \frac{z}{w}\]
    hence we just need to check that \(\sph^1 = \{e^{i\theta} \mid \theta \in \rr\}\) is a one dimensional submanifold of \(\cc\) and then the restriction of \(\mu\) on \(\sph^1\) is automatically smooth, which finishes the proof.
\end{proof}

In genral, the check of Lie group structure is based on the structure of manifold, many topological groups with manifold structure are Lie groups, It is can be concluded by \textbf{Hilbert's fifth problem}, An formal answer is given by Yamabe in 1953, we state it without proof such that we can quickly determine some Lie group in some cases:
\begin{theorem}[Neumann-Yamabe] $ \\$
    If \(G\) is a locally compact topological group, then the following statement are equivalent: \\
    (1) \(G\) is a Lie group.\\
    (2) \(G\) is locally euclidean.\\
    (3) \(G\) has the \textbf{no small subgroup (NSS)} properties: there is an open neighborhood \(U\) of \(e\) containing no non-trival subgroup.
\end{theorem}

\begin{remark}
    The theorem is based on the paper (ref: \url{https://www.math.uci.edu/~isaac/Taobookreview-final.pdf?utm_source=chatgpt.com}), another reference is by Terence Tao's note (ref: \url{https://terrytao.wordpress.com/wp-content/uploads/2012/03/hilbert-book.pdf?utm_source=chatgpt.com}).
    here are some immediate comments:\\

    (1) \(\sph^1\) is a Lie group since it is compact and a one-dimensional manifold. (A Chinese saying: 杀鸡用牛刀...)\\

    (2) A fact is that p-adic number \(\qq_p\) is locally compact since \(\zz_p\) is compact by theorem of Tychonoff, but it is not Lie group by condition NSS: we have a deceasing sequence of subgroups
    \[\zz_p \supseteq p\zz_p \supseteq p^2\zz_p \supseteq \cdots\]
    with a comparsion \(p^k\zz_p = \{x \in \qq_p \mid |x|_p \leq e^{-k}\} \subset  B_p(0,e^{-k+1}) \) for any \(k\).\\

    In particular \(\rr\) is a lie group under addition although it is very plain and trival, but by comparing to \(\qq_p\) we see that \(\rr\) has NSS property: For example take \((-1,1)\), then any non-trival subgroup of \(\rr\), if we take some non-zero element \(x\), then there exists \(n \in \nn\) such that \(|nx|>1\) (it is the archimedean property of \(\rr\)!).
\end{remark}

with the help of the nice theorem, we can state some examples of Lie group without proof with some knowledge of their topology:
\begin{example}
    Here are some examples of Lie group:\\
        (1) The additive group \((\rr^n,+)\) is a Lie group, similarly if we see \(\cc\) as a real manifold, then \((\cc^n,+)\) is also a 2n-dimension real Lie group.\\

        (2) \(\sph^1\) is a Lie group, but generally \(\sph^n\) is not necessary a Lie group, becasue not all \(\sph^n\) can be well-defined as a topogical group. Similar situation holds for projective space \(\rp^n\), and diffeomrphsim \(\rp^1 \cong \sph^1\) ensures projective line can be seen as a Lie group.\\

        (3) The n-dimensional torus \(T^n\) is a Lie group by the finite product of Lie group \(\sph^1 \times \dots \times \sph^1\).\\

        (4) The general linear group \(\mathrm{GL}(n,\rr)\) is a Lie group under the martix multiplication.

        (5) Affine group \(A(n,\rr)\) is a Lie group by isomorphsim with \(\mathrm{GL}(n,\rr)\times\rr^n\):
        \[(x\mapsto AX+b) \longleftrightarrow (A,b)\]
\end{example}
...
\subsection{Some example of duality}

\begin{problem}
    If \(G\) is a finite abelian group with a subgroup \(H\), then we can prove that \(G/H\) is isomorphic to some subgroup of \(G\), or equivalently a short exact sequence
    \[0 \to H \to G \to G/H \to 0\]
    splits. We will prove the result by \textbf{Pontrjagin duality}. Formally, we define the dual group of an abelian group \(G\) as
    \[G^* = \Hom_{\zz}(G, \qq/\zz)\]
\end{problem}

\begin{proof}
    It is just the application of injective module, the outline of the proof is as following:\\
    (1) \(\qq/\zz\) is an injective \(\zz\)-module, hence we can conclude a exact sequence
    \[0 \to (G/H)^* \to G^* \to H^* \to 0\]
    (2) Check that the dual group is enough big: for any element \(a \in G\), there exists a homomorphism \(f \in G^*\) such that \(f(a) \neq 0\). (May be it is also necessary to check that \(G^*\) is a well-defined abelian group althogh it is immediate by abelian category.)\\
    (3) Prove \(G^* \cong G\) if \(G\) is finite abelian: Starting from the case \(G = \zz/n\zz\), then conclude by the fundamental theorem of finite abelian group.\\
    (4) Applying the result of (3) to (1), then immediate we can conclude.\\

    \textbf{Here some details are given:}\\
    (1) In the sense of \(\zz\)-module, \(\qq/\zz\) is an injective module if and only if it is a divisible group, it is easy to check that \(\qq\) is divisible, then \(\qq/\zz\) is also divisible since any quotient of divisible group is again divisible.\\
    (2) When \(a\) is of order \(n<+\infty\), we can define a homomorphism 
    \[f: \langle a \rangle \to \qq/\zz, \quad a \mapsto \frac{1}{n} + \zz\]
    it is well-defined on the group generated by \(a\), it can be seem as a \(\zz\)-submodule of \(G\), hence we can lift \(f\) to a homomorphism \(\hat{f}: G \to \qq/\zz\) such that \(\hat{f}|_{\langle a \rangle} = f\), it can be done becasue \(\qq/\zz\) is injective. Similarly when \(a\) is of inifinite order, we just define
    \[f: \langle a \rangle \to \qq/\zz, \quad a \mapsto \frac{1}{2}+\zz\]
    then similarly we can lift \(f\).\\
    (3) When \(G = \zz/n\zz\), we set \(\phi \in G^*\), then it is necessary that \(\phi(\bar{0}) = \zz\), hence we just need to determine \(\phi(\bar{1})\), by the order we have 
    \[n\phi(\bar{1}) = \phi(n\bar{1}) = \phi(\bar{0}) = \zz\]
    which shows that \(x \in \qq\) must be a number such that \(nx\) is an integer, hence we can caculate all cases that \[\phi(\bar{1}) = \frac{k}{n} + \zz, \quad k=0,1,...,n-1\] 
\end{proof}
