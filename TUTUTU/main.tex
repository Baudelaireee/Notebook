...
\begin{itemize}
    \item We try to see dual as some function, which maps a vector space to another vector space (by proposition \ref{dualvectorspace}), so we naturally define \(\mathbf{Vect}_K\) to be the set of all vector spaces over field \(K\), then dual can be seen as a function in it
