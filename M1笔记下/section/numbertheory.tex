...
This theory corresponds to the geometry of numbers, the idea is to consider embedding of number fields into \(\rr\) or \(\cc\), we now consider a number field \(K\) of degree \(n\), and review some basic notations:
    ...
    \begin{theorem}[Minkowski's convex theorem] $ \\$
        Let \(K\) be a number field of degree \(n\) with ring of integers \(\ok\), and let \(\iota(S) \subset K_{\infty}\) be a convex symmetric measurable subset with \(\mathrm{Vol}(\iota(S)) > 2^n \cdot \mathrm{covol}(\iota(\ok))\), then there exists a non-zero element \(x \in \ok\) such that \(\iota(x) \in S\).
    \end{theorem}
    \begin{proof}
...
\begin{colorlemma}
    ...
    \[N(I) \leq (\frac{2}{\pi})^s\sqrt{|d_K|}\]
where \(d_K\) is the discriminant of \(K\), thus \(J_K/P_K\) is exactly a finite abelian group, and we call it the \textbf{class group} of \(K\), and its order is called the \textbf{class number} of \(K\).
\end{colorlemma}
\begin{proof}
    ...
    \[|N_{K/\qq}(x)| \leq (\frac{2}{\pi})^s\sqrt{|d_K|} N(y\mathfrak{a^{-1}})\]
