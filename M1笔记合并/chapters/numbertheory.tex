...
\begin{colorexample}
    If \(P(X) = X^n+aX+b\), the the discriminant of the polynomial is 
    \[\mathrm{Disc}(P) = (-1)^{n(n-1)/2}[n^nb^{n-1} + (1-n)^{n-1}a^n]\]
\end{colorexample}
...
\begin{itemize}
    ...
    \item Define the norm map (similar with the adele norm)
...
\item The kernel of \(l\) is the group of roots of unity  \(\mu(K)\), hence it motivates us to restrict the map \(l\) on \(\ok^\times\) for the structure, and it is not difficult to verify the following equivalent condition:
    \[x \in \ok^\times \iff \mathrm{N}_{K/\mathbb{Q}}(x) = \pm 1 \iff N(\iota(x)) =  1 \iff T(l(x)) = 0\]
    Hence it motivates us to define the \textbf{hyperplane}
    \[H := \{x \in \mathbb{L} \mid x_1 + \cdots + x_{r+s} = 0\} \]
    it is a subspace of \(\mathbb{L}\) of dimension \(r+s-1\), and \textbf{norm-one surface}
    \[S = \{x \in K_\infty \mid N(x) = 1\}\]
    it is a closed subgroup of minkowski space \(K_{\infty }\).
    
\end{itemize}

    \end{remark}

\begin{colortheorem}[Dirichlet]
    \(l(\mathcal{O}_K^\times)\) is a complet lattice of \(\mathbb{L}\), and the structure of unit group can be given by isomorophism
    \[\oo_K^\times \cong \mu(K) \times \zz^{r+s - 1}\]
\end{colortheorem}
...
The lebesgue volume of fundamental domain \(\mathbb{L}/ l(\oo_K^\times)\) is called the \textbf{regulator} of number field \(K\), and we usually denote it by \(d_K\); The generators of the free module \(l(\oo_K^\times)\) is called \textbf{fundamental unit} of \(K\), and if \(\{\varepsilon_1,\dots,\varepsilon_{r+s-1}\}\) is a basis, then each unit can be written as the form 
\[u \cdot \varepsilon_1^{m_1} \cdots \varepsilon_{r+s-1}^{m_{r+s-1}}\]
where \(u \in \mu(K)\).
...
\subsubsection{Dirichlet convolution and multiplicative functions}
...
\begin{proposition}[Euler's product]
    Supppose \(f\) is an arithmetic function such that the series \(\sum_{n \geq 1} f(n)\) converges absolutely, then \(\prod_{p} \sum_{k \geq 0}f(p^k)\) converges abolutely, and

    -If \(f\) is multpilicative,  \(\sum_{n \geq 1} f(n) =  \prod_{p}\sum_{k \geq 0} f(p^k)\).

    -If \(f\) is completely multiplicative, then \(|f(p)| < 1\) for any prime \(p\) and 
    \[\sum_{n \geq 1} f(n) = \prod_{p} \frac{1}{1-f(p)}\]
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{assets/lower/cover/hyperbola.jpg}
    \caption{example \(x =2 \)}
    \label{fig:mylabel}
\end{figure}
...
\begin{colortheorem}[Hamadard,1896]
...
\begin{colorcorollary}
    The non-trival zeros of zeta function lie in the \textbf{critical strip} \(\{s \in \cc: 0 < \Re(s) < 1\}\).
\end{colorcorollary}

\subsubsection{Von Mangoldt function}
...
\[\Lambda(n) := \begin{cases}
    ...
    0 \quad & \text{otherwise}
\end{cases}\]
Notice that \(\Lambda(1) = 0\) and it is only related to the prime numbers, and we view it in the context of arithemetic functions, it is not hard to prove the following properties:
...
\textcolor{blue}{This function appears in the Riemann's paper first time although it is not named as such.}



\begin{lemma}[Riemann,1859]
    For any \(\Re(s)>1\), we have
    \[\log \zeta(s) = \sum_{n \geq 2} \frac{\Lambda(n)}{\log(n)} \frac{1}{n^s}\]
    and
    \[-\frac{\zeta(s)}{\zeta(s)} = \sum_{n \geq 2}\frac{\Lambda(n)}{n^s} ,\quad \mathrm{Re}(s) > 1\]
\end{lemma}

\begin{proof}
    Take the logarithm to zeta function of the Euler product expansion, we have
    \[\log \zeta(s) = - \sum_{p} \log(1-p^{-s})\]
    notice that \(\log(1-x)\) has a power series expansion if \(|x|<1\), and here \(|p^{-s}| =|p|^{-\mathrm{Re}(s)} <1\), so we can write
    \[\log \zeta(s) = \sum_{p} \sum_{k\geq 1} \frac{1}{k} p^{-ks}\]
    Notice that \(1\) is not a prime by convention, and \(\Lambda\) only supports on the prime powers, so the sum can be rewritten as 
...
\end{proof}
 ...
 and we call it \textbf{the first Tchebychev function}, and \textbf{the second Tchebychev function} is defined by
\[\psi(x) := \sum_{n \leq x} \Lambda(n)\]
they are equivalent in the sense of asymptotic behavior, and he estimated the growth of \(\theta\), which can be translated as following:

\begin{lemma}[Tchebychev, 1850]
    \(\psi(x) \asymp x\), that means there exists two positive constants \(c_1,c_2\) such that for any \(x\) large enough, we have
...
\begin{corollary}
   \(\psi\) and \(\theta\) have the same asymptotic behavior.
\end{corollary}
...
\[\pi(x) = \sum_{p \leq x}1 \]
\begin{colorclaim}
    As \(x \to \infty\), the following asymptotic behavior are equivalent:
    \begin{enumerate} [(a)]
        \item \(\pi(x)/x \sim 1/\log x \) 
        \item \(\psi(x) \sim x\) 
...
\subsection{Dedekind L-function}
